function plotQuantNumbers (fDimensionsInCm, cOutputPath)
...
function [x, y, yq, yTicks, yTickLabels] = getData ()
    ...
    yTicks      = 0:2^iWordLength-1;
    yTickLabels = dec2bin ([yTicks(2^(iWordLength-1)+1:(2^(iWordLength))) yTicks(1:2^(iWordLength-1))], iWordLength);
    yTicks(end+1)= 2^iWordLength;
    yTicks      = (yTicks - 2^(iWordLength-1))/(2^(iWordLength-1));
